...
public class SortArray
{
    ...
    * @return The index of the pivot */
    ...
    /** 
 * Recursively find the kth item in a sublist of items
 * @param a An array of Comparable objects.
 * @param k An integer >= 0 that is the position to return (1-based index).  
 * @param first An integer >= 0 that is the index of the first
 * array entry to consider.
 * @param last An integer >= 0 that is the index of the last
 * array entry to consider.
...
T kthItem(T[] a, int k, int first, int last) {
    ...
    // Calculate the 0-based position of the pivot in the sorted array
    int pivotPosition = pivotIndex - first + 1;

    if (k == pivotPosition) {
        // If the pivot is the k-th element, return it
        return a[pivotIndex];
    } else if (k < pivotPosition) {
        // If the k-th element is in the left partition
        return kthItem(a, k, first, pivotIndex - 1);
    } else {
        // If the k-th element is in the right partition
        return kthItem(a, k - pivotPosition, pivotIndex + 1, last);
    }
}

/**
 * Partition the array around a pivot such that elements smaller than or equal to
 * the pivot are on the left, and elements greater than the pivot are on the right.
 * @param a An array of Comparable objects.
 * @param first The starting index of the sublist to partition.
 * @param last The ending index of the sublist to partition.
 * @return The index of the pivot after partitioning.
 */
private static <T extends Comparable<? super T>> int partition(T[] a, int first, int last) {
    T pivot = a[last]; // Choose the last element as the pivot
    int i = first - 1;
